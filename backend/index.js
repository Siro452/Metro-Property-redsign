...
app.get("/PropertyListing", async (req, res) => {
  ...
  try {
    const individualListing = await Rental.where("_id").equals(
      reqQuery.property
    );
...
// Getting similar properties carousel
app.get("/SimilarPropertiesSliced", async (req, res) => {
  // Add a constant to extract the skip value
  //Add a function to remove the skip value from the query.
  console.log(`this is the similar sliced ${req.query}`);
  //Shallow copying to modify the query
  let reqQuery = { ...req.query };
  //Turn the request into a json string so that $ symbols can be added with the replace function
  let queryStr = JSON.stringify(reqQuery);
  ...
  console.log(`this is just before query similar ${parsedQueryStr}`);
  try {
    const returnedResults = await Rental.find(parsedQueryStr).limit(3).skip();
    res.status(200).send(returnedResults);
  } catch (e) {
    console.log(e.message);
  }
