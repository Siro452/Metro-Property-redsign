...
app.get("/testparams", async(req, res) => {
    ...
    //Filtering out results where the fields have been set to any
    // reqQuery = Object.fromEntries(
    //     Object.entries(reqQuery).filter(([key, value]) => value !== "any" | value[value] !== "any")
    // );
    ...
    console.log(`this is once it is stringified ${queryStr}`)
    
    
   queryStr = queryStr.replace(/\b(gt|gte|lt|lte|in)\b/g, (match) => `$${match}`)
    
   console.log(`this is once the $ is added ${queryStr}`)
    let parsedQueryStr = JSON.parse(queryStr)
    console.log(`this is once it is parsedback ${parsedQueryStr}`)
    
    try{
        const orange = await Rental.find(parsedQueryStr)
        // const orange = await Rental.find(queryStr)
         res.send(orange)
        //  res.send(req.query)
        //  res.send(parsedQueryStr)
        // console.log(orange)
    ...
    //Adding any $ around $gte queries requests



    // const banana =  await Rental.find(req.query)
    // console.log(banana)
